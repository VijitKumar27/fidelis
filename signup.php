...
$usernm = $passwrd = $confirm_password = $name= $pol_id= $branch="";
$nameErr = $branchErr = $pol_idErr = $confirm_password_err = $passwrdError = $usernameErr ="";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  else
  {
    ...
    else
    {
      ...
      $sql="SELECT * FROM pol_login";
      $query=mysqli_query($conn, $sql);
      
      while($row=mysqli_fetch_assoc($query))
      {
          $table_username = $row['usernm'];
          if ($usernm==$table_username)
          {
  ...
  if (empty($_POST["passwrd"])) {
    $passwrdError = "password cannot be empty";
  } else {
    ...
    if (empty($_POST["name"])) {
      ...
      if (empty($_POST["branch"])) {
        $branchErr = "branch cannot be empty";
      } else {
        $branch = test_input($_POST["branch"]);
      }
      if (empty($_POST["pol_id"])) {
        $pol_idErr = "name cannot be empty";
      } else {
        $pol_id = test_input($_POST["pol_id"]);
      }
    // Check input errors before inserting in database
    if(empty($usernameErr) && empty($passwrdError) && empty($nameErr) && empty($pol_idErr) && empty($branchErr)){
        if (!$conn) {
            die("Connection failed: " . mysqli_connect_error());
          } 
          else
          {
        $sql = "INSERT INTO pol_login (usernm, passwrd, name, branch, pol_id)
          VALUES ('$usernm', '$passwrd', '$name', '$branch', '$pol_id')";
  ...
  <div class="container">
    ...
    <div class="bg-white h-100 p-4 shadow">
            ...
            <div class="form-group">
                ...
                <input type="text" name="username" class="form-control <?php echo (!empty( $usernameErr)) ? 'is-invalid' : ''; ?>" value="<?php echo $usernm; ?>">
                <span class="invalid-feedback"><?php echo  $usernameErr; ?></span>
            </div>    
            <div class="form-group">
                <label>Password</label>
                <input type="password" name="passwrd" class="form-control <?php echo (!empty($passwrdError)) ? 'is-invalid' : ''; ?>" value="<?php echo $passwrd; ?>">
                <span class="invalid-feedback"><?php echo $passwrdError; ?></span>
            </div>
            ...
            <div class="form-group">
                <label>Branch</label>
                <input type="text" name="branch" class="form-control <?php echo (!empty($branchErr)) ? 'is-invalid' : ''; ?>" value="<?php echo $branch; ?>">
                <span class="invalid-feedback"><?php echo $branchErr; ?></span>
            </div> 
            <div class="form-group">
                <label>Police ID</label>
                <input type="text" name="pol_id" class="form-control <?php echo (!empty($pol_idErr)) ? 'is-invalid' : ''; ?>" value="<?php echo $pol_id; ?>">
                <span class="invalid-feedback"><?php echo $pol_idErr; ?></span>
            </div> 
            <div class="form-group">
